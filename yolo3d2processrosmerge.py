...
class rosCamera():
    def __init__(self, ):
        ...
        self.imagesub = rospy.Subscriber('/apollo/sensor/camera/obstacle/front_6mm', msgImage, self.callback)
        self.boxpub = rospy.Publisher('/apollo/sensor/camera/yolobox', String, queue_size=10)
    ...
    def callback(self, data):
        ...
        #image = yolo.detect_image(image)
        image, objs = detect_3d_box(self.yolo, image, cv_image)#'007517.png')
        objs_str = "%d:%d"%(data.header.stamp.secs, data.header.stamp.nsecs)
        for obj in objs:
             objs_str = objs_str+" %d,%f,%f,%f,%f,%f,%f,%f,%f"%(obj[0], obj[1], obj[2], obj[3], obj[4], obj[5], obj[6], obj[7], obj[8])
        msg = String(objs_str)
        print(objs_str)
        self.boxpub.publish(msg)
        result = np.asarray(image)
        curr_time = timer()
        ...
        if self.accum_time > 1:
            ...
            self.curr_fps = 0
        cv2.putText(result, text=self.fps, org=(3, 15), fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                    fontScale=0.50, color=(255, 0, 0), thickness=2)
        cv2.namedWindow("result", cv2.WINDOW_NORMAL)
        cv2.imshow("result", result)
        if cv2.waitKey(1) & 0xFF == ord('q'):
...
#    detect_3d_box(YOLO(), 'testout', '000008.png')
    rospy.init_node('yolo3ddetect', anonymous=True)
