...
#    x = resblock_body(x, 1024, 4)
    x = DarknetConv2D_BN_Leaky(20, (3,3))(x)
    x = resblock_body(x, 40, 1)
    x = resblock_body(x, 80, 2)
    x = resblock_body(x, 160, 3)
    x = resblock_body(x, 320, 2)
    x = resblock_body(x, 640, 2)
    

    return x

def make_last_layers(x, num_filters, out_filters, d3dim, d3orient):
   ...
   d3dimy = compose(
                DarknetConv2D_BN_Leaky(num_filters, (3,3)),
                            DarknetConv2D(d3dim, (1,1)))(x))
   d3orienty =  compose(DarknetConv2D_BN_Leaky(num_filters, (3,3)),
                       DarknetConv2D(d3orient, (1,1)))(x))
...
def yolo_body(inputs, num_anchors, num_classes):
    ...
    x, y1, d3dimy1, d3orienty1 = make_last_layers(darknet.output, 512, num_anchors*(num_classes+5), 3, 3*BIN)
    ...
    x, y2, d3dimy2, d3orienty2 = make_last_layers(x, 256, num_anchors*(num_classes+5), 3, 3*BIN)

    x = compose(
            DarknetConv2D_BN_Leaky(128, (1,1)),
            UpSampling2D(2))(x)
    x = Concatenate()([x,darknet.layers[57].output])
    x, y3, d3dimy3, d3orienty2 = make_last_layers(x, 128, num_anchors*(num_classes+5), 3, 3*BIN)
...
def yolo_eval(yolo_outputs,
    ...
    for l in range(num_layers):
        _boxes, _box_scores = yolo_boxes_and_scores(yolo_outputs[0][l],
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, print_loss=True):
    ...
    for l in range(num_layers):
        ...
        class_loss = object_mask * K.binary_crossentropy(true_class_probs, raw_pred[...,14:], from_logits=True)
        ...
        d3orient_loss = orientation_loss(raw_true_3d_cossin, box_3d_cossin, object_mask, mf) 
            
        xy_loss = K.sum(xy_loss) / mf
        wh_loss = K.sum(wh_loss) / mf
        confidence_loss = K.sum(confidence_loss) / mf
        class_loss = K.sum(class_loss) / mf
        d3conf_loss = K.sum(d3conf_loss) / mf
        d3dim_loss = (K.sum(d3dim_loss) / mf)*4.0
       ...
       # + d3orient_loss
        ...
        if print_loss:
            loss = tf.Print(loss, [loss, xy_loss, wh_loss, confidence_loss, class_loss, d3conf_loss, d3dim_loss, d3orient_loss], message='loss: ')
