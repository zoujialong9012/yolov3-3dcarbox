...
def yolo_body(inputs, num_anchors, num_classes):
    ...
    x, y1 = make_last_layers(darknet.output, 512, num_anchors*(num_classes+5+3+3*BIN))
    ...
    x, y2 = make_last_layers(x, 256, num_anchors*(num_classes+5+3+3*BIN))

    x = compose(
            DarknetConv2D_BN_Leaky(128, (1,1)),
            UpSampling2D(2))(x)
    x = Concatenate()([x,darknet.layers[57].output])
    x, y3 = make_last_layers(x, 128, num_anchors*(num_classes+5+3+3*BIN))
...
def yolo_boxes_and_scores(feats, anchors, num_classes, input_shape, image_shape):
    ...
    box_cossin = K.reshape(box_3d_cossin, [-1, 2, 2])
...
def yolo_eval(yolo_outputs,
    ...
    for l in range(num_layers):
        _boxes, _box_scores, _box_dim, _box_3d_conf, _box_cossin = yolo_boxes_and_scores(yolo_outputs[l],
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, print_loss=True):
    ...
    for l in range(num_layers):
        ...
        d3orient_loss = 10.0 * orientation_loss(raw_true_3d_cossin, box_3d_cossin, object_mask, mf) 
