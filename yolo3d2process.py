...
def detect_3d_box(yolo, box3d_loc, img):
#def detect_3d_box(box3d_loc, img):
    tfconfig = tf.ConfigProto(allow_soft_placement=True)
    tfconfig.gpu_options.allow_growth = True

    #graph=tf.Graph()
    sess = tf.Session(config=tfconfig) #, graph=graph)
...
#    saver=tf.train.import_meta_graph('/home/cidi/dl/3dobject/3D-Deepbox/model/model-180.meta')
#    saver.restore(sess, '/home/cidi/dl/3dobject/3D-Deepbox/model/model-180')
...
#    model_file=tf.train.latest_checkpoint('/home/cidi/dl/3dobject/3D-Deepbox/model')
   ...
   # ckpt = tf.train.get_checkpoint_state('./model/')
    #print(tf.train.latest_checkpoint('/home/cidi/dl/3dobject/3D-Deepbox/model')) 
    #saver.restore(sess, '/home/cidi/dl/3dobject/3D-Deepbox/model/model-180')
    #tf.train.latest_checkpoint('/home/cidi/dl/3dobject/3D-Deepbox/model'))
    #sess.run("3dbox")
...
#        os.mkdir(box3d_loc)
   ...
   # # oimage = oimage.astype(np.float32, copy=False)
    ...
    oimage = cv2.imread(img)
  #  out_boxs = [[ 178.4899,     595.38495,    252.34717,    712.2158   ],
  #              [ 176.31497,    879.29236,    233.65086,    960.1607   ],
  #              [ 179.32487,    329.2916,     351.98886,    636.3809   ],
  #              [ 167.49907,    743.5531,     198.94377,    794.86584  ],
  #              [ 176.30672,    915.4688,     233.04782,   1019.40857  ],
  #              [ 191.64938,      7.4311576,  363.78525,    335.79     ],
  #              [ 202.9137,     977.08386,    350.2626,    1254.9988   ]]
  #  out_classes=['Car', 'Car', 'Car', 'Car', 'Car', 'Car', 'Car']
  #  print(out_boxs)
  #  image1 = np.asarray(image)
  #  print(image1.shape)
    #image = oimage.astype(np.float32, copy=False)
    ih = oimage.shape[0]
    iw = oimage.shape[1]
    print(out_boxs)
    ...
    #    print(patch.shape)
      ...
      #  line = str(line)+' '.join(str(np.max(prediction[2][0]))) + str(angle_offset)+'\n'
        #box3d.write(line)
...
if __name__ == '__main__':
    ...
    #yolop.run()
    detect_3d_box(YOLO(), 'testout', '007437.png')#'007517.png')
