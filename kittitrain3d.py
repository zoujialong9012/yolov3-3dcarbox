...
#from yolo3.modelmerge import preprocess_true_boxes, yolo_body, tiny_yolo_body, yolo_loss
#from yolo3.modelmergetree import preprocess_true_boxes, yolo_body, tiny_yolo_body, yolo_loss
from yolo3.modelmergetree import preprocess_true_boxes, yolo_body, tiny_yolo_body, yolo_loss
   ...
   # input_shape = (416,416) # multiple of 32, hw
    ...
    if is_tiny_version:
        ...
        model = create_model(input_shape, anchors, num_classes,
            freeze_body=0, load_pretrained=True,
            weights_path='model_data/yolo_weights.h5') # make sure you know what you freeze
            #weights_path='logs/000/ep015-loss22.413-val_loss23.012.h5')
            #weights_path='logs/000/ep066-loss29.753-val_loss29.087.h5')
            #ep069-loss18.563-val_loss18.055.h5')
            #'model_data/yolo_weights.h5') # make sure you know what you freeze
...
def create_model(input_shape, anchors, num_classes, load_pretrained=True, freeze_body=2,
    ...
    y_true = [Input(shape=(h//{0:32, 1:16, 2:8}[l], w//{0:32, 1:16, 2:8}[l], \
        num_anchors//3, num_classes+5+3+3*BIN)) for l in range(3)]

    model_body = yolo_body(image_input, num_anchors//3, num_classes)
    print('Create YOLOv3 model with {} anchors and {} classes.'.format(num_anchors, num_classes))

    plot_model(model_body, to_file='modelmergetreetrain.png')
    if load_pretrained:
    ...
    #    print('load rand weights =================')
    #    rlayer = model_body.get_layer('conv2d_50')
    #    #weight = rlayer.get_weights()
    #    rlayer.set_weights([np.random.randn(*weight.shape)*10 for weight in rlayer.get_weights()])
    #    rlayer = model_body.get_layer('conv2d_48')
    #    rlayer.set_weights([np.random.randn(*weight.shape)*10 for weight in rlayer.get_weights()])
    #    rlayer = model_body.get_layer('leaky_re_lu_44')
    #    rlayer.set_weights([np.random.randn(*weight.shape)*10 for weight in rlayer.get_weights()])
    #    rlayer = model_body.get_layer('leaky_re_lu_45')
    #    rlayer.set_weights([np.random.randn(*weight.shape)*10 for weight in rlayer.get_weights()])
    #    rlayer = model_body.get_layer('leaky_re_lu_54')
    #    rlayer.set_weights([np.random.randn(*weight.shape)*10 for weight in rlayer.get_weights()])
    #    rlayer = model_body.get_layer('leaky_re_lu_52')
    #    rlayer.set_weights([np.random.randn(*weight.shape)*10 for weight in rlayer.get_weights()])
    #    rlayer = model_body.get_layer('leaky_re_lu_35')
    #    rlayer.set_weights([np.random.randn(*weight.shape)*10 for weight in rlayer.get_weights()])
    #    rlayer = model_body.get_layer('leaky_re_lu_36')
    #    rlayer.set_weights([np.random.randn(*weight.shape)*10 for weight in rlayer.get_weights()])
...
def create_tiny_model(input_shape, anchors, num_classes, load_pretrained=True, freeze_body=2,
    ...
    y_true = [Input(shape=(h//{0:32, 1:16}[l], w//{0:32, 1:16}[l], \
        num_anchors//2, num_classes+5+3+3*BIN)) for l in range(2)]
