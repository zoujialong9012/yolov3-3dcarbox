...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, print_loss=True):
    ...
    for l in range(num_layers):
       ...
       # print(box_3d_cossin.shape)
        ...
        d3orient_loss = 0.00001*orientation_loss(raw_true_3d_cossin, box_3d_cossin, object_mask, mf) 
            
        xy_loss = K.sum(xy_loss) / mf
        wh_loss = K.sum(wh_loss) / mf
        confidence_loss = K.sum(confidence_loss) / mf
        class_loss = K.sum(class_loss) / mf
        d3conf_loss = (K.sum(d3conf_loss) / mf)*0.00001
        d3dim_loss = (K.sum(d3dim_loss) / mf)*0.00001
